...
public class OpenIDServerAssociationStore extends
	...
	public OpenIDServerAssociationStore(String associationsType) {
		...
		counter = 0;
		cache = AssociationCache.getCacheInstance();
	...
	public synchronized Association load(String handle) {
		...
		association = cache.getFromCache(handle);

		// if failed, look in the database
		if (association == null) {
			log.info("Association " + handle
		...
		if (association == null) {
			log.info("Association " + handle + " not found in the database.");
			return null;
		}
	...
	public synchronized void remove(String handle) {
		...
		log.info("Removing the association" + handle + " from the database");
