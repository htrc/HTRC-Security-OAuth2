...
<%
    ...
    if(matchId != null && !matchId.trim().equals("") && attributeValue != null &&
        ...
        matchElementDTO.setMatchElementName(EntitlementPolicyConstants.ENVIRONMENT_ELEMENT);
        ...
        if(attributeId != null && !attributeId.trim().equals("") &&
           ...
           !attributeDesignatorDataType.trim().
            ...
            designatorDTO.setElementName(EntitlementPolicyConstants.ENVIRONMENT_ELEMENT);
...
<script type="text/javascript">
    ...
    function deleteMatchElement(matchElementId) {
        location.href = 'delete-match.jsp?subElementName=Environment&matchElementId=' + matchElementId +
                        '&editSubElement=' + <%=editSubElement%> ;
    }

    function doCancel() {
        location.href = 'cancel-match.jsp?subElementName=Environment';
    }

    function doCancelMatch() {
        location.href = 'add-environments.jsp?editSubElement=' + <%=editSubElement%>;
    }

    function doAdd () {
        var subElementId = "<%=entitlementPolicyBean.getCurrentSubElementNumber()%>";
        location.href = 'add-target.jsp?subElementName=Environment&subElementId=' + subElementId +
                        '&editSubElement=' + <%=editSubElement%>;
    }
    ...
    function doAddMatch () {
        ...
        if(doValidation()){
            document.matchElementForm.action = "add-environments.jsp?matchElementId=" + '<%=matchElementId%>'
...
<div id="middle">
    ...
    %>
        <h3 class="page-subtitle"><fmt:message key='edit.environment.element'/></h3>
    <%
        } else {
    %>
        <h3 class="page-subtitle"><fmt:message key='add.environment.element'/></h3>    
    <%
	    }
    %>
    ...
    <div class="tabTen">
        ...
        <tr>
            <td><fmt:message key="environment.match"/>  <%=existingMatchElement.getElementId()%></td>
            <td><%=existingMatchElement.getMatchId()%></td>
            <td>
                <a href="add-environments.jsp?subElementName=Environment&matchElementId=<%=existingMatchElement.getElementId()%>&editMatchElement=true&editSubElement=<%=editSubElement%>" class="icon-link" style="background-image:url(images/edit.gif);"><fmt:message key="edit"/></a>
        ...
        %>
        <tr class="noDataBox">
            <td colspan="3"><fmt:message key="no.environment.match.define"/><br/></td>
        </tr>
        <%
            }
        %>
