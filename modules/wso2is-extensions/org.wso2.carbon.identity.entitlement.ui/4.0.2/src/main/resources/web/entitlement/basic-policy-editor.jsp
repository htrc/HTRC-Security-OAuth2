...
<script type="text/javascript">
    ...
    function preSubmit(){
        ...
        if(document.getElementById('actionRuleTable') != null){
            resourceRuleTable = jQuery(document.getElementById('actionRuleTable').rows[document.
    ...
    function createNewActionRuleRow() {
        var rowIndex =  jQuery(document.getElementById('actionRuleTable').rows[document.
                            getElementById('actionRuleTable').rows.length-1]).attr('data-value');
        var index = parseInt(rowIndex, 10) + 1;
        jQuery('#actionRuleTable > tbody:last').append('<tr data-value="0"><td><table class="oneline-listing"><tr></tr><tr><td>This action</td>' +
                '<td><select id="operationRuleType_0" name="operationRuleType_0"  ><%for (String type  : operationTypes) {if (selectedRuleOperationType != null && type.equals(selectedRuleOperationType)) { %><option value="<%=type%>"  selected="selected"><%=type%></option><%} else {%><option value="<%=type%>"><%=type%></option><%}}%></select></td>' +
                '<td> be performed on resource which is </td>' +
                '<td><select id="resourceRuleFunction_0" name="resourceRuleFunction_0"><%for (String function : functions) {if (selectedRuleResourceFunction != null && selectedRuleResourceFunction.equals(function)) { %><option value="<%=function%>"  selected="selected"><%=function%></option><%} else {%><option value="<%=function%>"><%=function%></option><%}}%></select></td><td>to</td><td><%if (selectedRuleResourceValue != null && selectedRuleResourceValue.trim().length() > 0) {%><input type="text" name="resourceRuleValue_0" id="resourceRuleValue_0" value="<%=selectedRuleResourceValue%>"/><%} else {%><input type="text" name="resourceRuleValue_0" id="resourceRuleValue_0" /><%}%></td>' +
                '<td>by user whose</td>' +
                '<td><select id="userAttributeId" name="userAttributeId" ><%for (String userAttribute : userAttributeIds) {if (selectedUserAttributeId != null && userAttribute.equals(selectedUserAttributeId)) {%><option value="<%=userAttribute%>"  selected="selected"><%=userAttribute%></option><%} else {%><option value="<%=userAttribute%>"><%=userAttribute%></option><%}}%></select></td>' +
                '<td> is </td>' +
                '<td><select id="function" name="function"  class="leftCol-small"><%for (String function : functions) {if (selectedFunction != null && selectedFunction.equals(function)) {%><option value="<%=function%>"  selected="selected"><%=function%></option><%} else {%><option value="<%=function%>"><%=function%></option><%}}%></select></td>' +
                '<td> to </td>' +
                '<td><%if (selectedUserAttributeValue != null && selectedUserAttributeValue.trim().length() > 0) {%><input type="text" name="userAttributeValue" id="userAttributeValue" value="<%=selectedUserAttributeValue%>"/><% } else {%><input type="text" name="userAttributeValue" id="userAttributeValue" /><%}%></td>' +
                '<td><a onclick="removeRow(this)" style="background-image:url(images/delete.gif);" type="button" class="icon-link"></a></td>' +
                '</tr><tr></tr></table></td></tr>');
    }
...
<div class="goToAdvance">
    ...
    <table>
        <tr>
            ...
            <table id="mainTable">
                <tr>
                    <td><fmt:message key="policy.name"/></td>
                    <%
...
%>
        ...
        <tr>
            ...
            <table  id="resourceRuleTable" >
                ...
                <table class="oneline-listing">
                <tr>
                    <td style="white-space:nowrap;">Resource  which is</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td>User whose</td>
