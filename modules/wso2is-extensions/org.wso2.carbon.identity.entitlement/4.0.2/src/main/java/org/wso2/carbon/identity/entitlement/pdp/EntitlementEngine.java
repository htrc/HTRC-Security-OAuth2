...
public class EntitlementEngine {
	...
	private EntitlementEngine(Registry registry, int tenantId) throws IdentityException {
        ...
        // TODO read this via configuration
        balana = Balana.getInstance();

        // adding init value to cache
        decisionClearingCache.addToCache(tenantId, 1);
     ...
     * @return
     */
    public ResponseCtx evaluate(AbstractRequestCtx requestCtx){
    ...
    private String getFromCache(String  request, boolean simpleCache) {
		...
		if (pdpDecisionCachingInterval > 0) {
			...
			int hashCode = decisionClearingCache.getFromCache(tenantId);
            if (hashCode == 0) {
                decisionCache.clear();
	...
	public void clearDecisionCache(boolean updateAllNodes) {
        ...
        if(updateAllNodes){
            decisionClearingCache.invalidateCache(tenantId);
        }
	}
}