...
public class AuthnRequestProcessor {
    ...
    public SAMLSSORespDTO process(SAMLSSOAuthnReqDTO authnReqDTO, String sessionId,
        ...
        if (log.isDebugEnabled()) {
            log.debug("Processing SAML Request. Issuer '" + authnReqDTO.getId() + "'");
        }
        try {
            
            //validate issuer info.
            if (!isValidIssuer(authnReqDTO)) {
...
//            PermissionUpdateUtil.updatePermissionTree(tenantId);
    ...
    private boolean isValidIssuer(SAMLSSOAuthnReqDTO authReqDTO) throws IdentityException {
        ...
        try {
            ...
            if (acsUrl != null && !spDO.getAssertionConsumerUrl().equals(acsUrl)) {
                log.error("ALERT: Invalid Assertion Consumer URL value '" + acsUrl + "' in the " +
                        "AuthnRequest message from  the issuer '" + spDO.getIssuer() + "'. Possibly " +
